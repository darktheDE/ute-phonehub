...
export function useCartSync() {
  ...
  const showMergeNotification = useCallback((guestItems: unknown[], backendItems: unknown[]) => {
    toast.info(
  ...
  const mergeGuestCartToBackend = useCallback(async (guestItems: unknown[], showSuccessToast: boolean) => {
    try {
      const guestCartItems = (Array.isArray(guestItems) ? guestItems : []).map((it: any) => {
  ...
  }, [clearCart, clearGuestCartId, items, mergeGuestCartToBackend, setItems, showMergeNotification]);
  ...
  }, [isAuthenticated, user, getMergeStatus, setMergeStatus, clearCart, checkAndMergeCart, items, guestCartId]);
